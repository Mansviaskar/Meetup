  ...
  return (
    <aside className="w-full sm:w-80 h-full bg-white border-1 border-slate-200 flex flex-col z-30 shadow-2xl animate-in  slide-in-from-right duration-200"> 
                ...
                messages.map((msg, index)=> {
                    ...
                    return (
                        <div key={msg.id || index} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                            ...
                            <div className={`px-3.5 py-2.5 rounded-2xl max-w-[85%] text-sm leading-relaxed shadow-xs 
                                ${isMe ? "bg-primary text-white rounded-tr-none font-medium" 
        ...
        <form onSubmit={handleSubmit} className='p-3 border-t border-slate-200 bg-slate-50 flex items-center gap-2'>
            ...
            rounded-xl px-3.5 py-2.5 text-sm text-slate-900 placeholder-slate-400 outline-none transition-all shadow-xs'/>
