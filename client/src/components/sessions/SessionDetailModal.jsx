...
const SessionDetailModal = ({session, onClose}) => {
  ...
  return (
    <div className='fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4'>
        ...
        border border-slate-100 overflow-hidden animate-in fade-in zoom-in duration-150'>
            {/*Modal Header*/}
            <div className='p-6 border-b border-slate-100 flex items-center jutify-between'>
                <div>
            ...
            <div className='flex border-b border-slate-100 px-6 bg-slate-50/50'>
                <button onClick={()=> setActiveTab("Chat")}
                    className={`py-3 px-4 font-medium text-sm border-b-2 cursor-pointer transition-all 
                    ${activeTab === "chat" ? "border-primary text-primary" : "border-transparent text-slate-500 hover:text-slate-800"}`}>
                    Chat Transcript ({session.messages?.length || 0})
                </button>

                <button onClick={()=> setActiveTab("participants")}
                    className={`py-3 px-4 font-medium text-sm border-b-2 cursor-pointer transition-all 
                    ${activeTab === "participants" ? "border-primary text-primary" : "border-transparent text-slate-500 hover:text-slate-800"}`}>
