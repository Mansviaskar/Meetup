    ...
    <div className='bg-white/70 backdrop-blur rounded-3xl p-6 transition-all flex flex-col justify-between space-y-5 border border-slate-100/60 shadow-xs'>
        ...
        <div className='space-y-3'>
            ...
            <div className='flex items-center justify-between'>
                ...
                ${isEnded ? "bg-slate-500/5 text-slate-500" : "bg-emerald-500/5 text-emerald-500"}`}>
                    
                    <span className={`size-1.25 rounded-full 
        ...
        <div className='flex items-center justify-between gap-3 pt-2'>
            <button onClick={()=> onOpenDetails(session.id)}
                className='w-full bg-slate-400/10 hover:bg-slate-400/20 text-slate-800 font-mediium 
                py-2.5 px-4 rounded-full text-xs transition-all cursor-pointer text-center'>
                View Details
            </button>
            {!isEnded && (
